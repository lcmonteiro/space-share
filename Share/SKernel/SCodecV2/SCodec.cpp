...
//	typedef mt19937 Generator;
	 ...
	 *                                    utils                                                                            *
	 *------------------------------------------------------------------------------------------------*/
        static inline vector<size_t> Select(Generator& gen, size_t n, size_t total);
	/**
	 *  encode functions
	 */
	static inline void Combine(const Container& data, uint32_t seed, uint8_t field, uint8_t sparsity, Frame& out);
	/**
	 * decode functions
	 */
	static inline void Organize(Frame& field, Container& coef, Container& data);
	/**/
	static inline uint32_t Solve(uint32_t size, Container& coef, Container& data);
...
#define ENCODE(FIELD, SPARSITY, FRAMESIZE) do{                                                 \
                /**										\
		 ...
		 */                                                                             \
                ...
                Combine(data, seed, field, sparsity, comb);                                     \
		/**										\
		 * add seed									\
		 */										\
		 ...
		 * normal density
		 */
		for (unsigned int i = highDensity; i < size; i++) {
			ENCODE(stamp[uint8_t(seed)].first, stamp[uint8_t(seed)].second, FRAME_SIZE);
		}
		/**
		 * high density
		 */
		for (unsigned int i = 0; i < highDensity; i++) {
			ENCODE(UINT8_MAX, UINT8_MAX, FRAME_SIZE);
		}
		/**/
		return size;
	}
	...
	uint32_t SCodec::Decode(
		...
		for (auto it = code.begin(); it != code.end(); ++it) {
			...
			Generator c(seed);
			/**/
			Frame coef(capacity, capacity + sizeof (int), 0);
			/**/
			for (register auto i : Select(c, capacity, sparsity)) {
                                coef[i] = uint8_t(c()) & field;
                        }
                        /**
			 * update state
			 */
	 ...
	 * encoder
	 *------------------------------------------------------------------------------------------------*/
	SCodec::Encoder& SCodec::Encoder::push(Frame&& data) {
		/**
		 * add data
		 */
	...
	SCodec::Encoder& SCodec::Encoder::push(Container&& data) {
		/**
		 * add data
		 */
		for (auto& d : data) {
	...
	Container SCodec::Encoder::pop(uint32_t highDensity) {
		Container out;
		/**
		 * encode
		 */
	 ...
	 * decoder
	 *------------------------------------------------------------------------------------------------*/
	SCodec::Decoder& SCodec::Decoder::push(Frame&& data) {
		/**
		 * decode
		 */
		__size = SCodec::Decode(Container({move(data)}), __capacity, __data, __coef, __field, __stamp.get());
		/**
		 */
		return *this;
	}

	SCodec::Decoder& SCodec::Decoder::push(Container&& data) {
		/**
		 * decode
		 */
	...
	static const int V2P[] = {
		512, 0, 1, 25, 2, 50, 26, 198, 3, 223, 51, 238, 27, 104, 199, 75, 4, 100, 224, 14, 52, 141, 239, 129, 28,
		...
		82, 41, 157, 85, 170, 251, 96, 134, 177, 187, 204, 62, 90, 203, 89, 95, 176, 156, 169, 160, 81, 11, 245,
	...
	static const int P2V[] = {
		...
		110, 220, 165, 87, 174, 65, 130, 25, 50, 100, 200, 141, 7, 14, 28, 56, 112, 224, 221, 167, 83, 166, 81, 162,
		89, 178, 121, 242, 249, 239, 195, 155, 43, 86, 172, 69, 138, 9, 18, 36, 72, 144, 61, 122, 244, 245, 247,
		243, 251, 235, 203, 139, 11, 22, 44, 88, 176, 125, 250, 233, 207, 131, 27, 54, 108, 216, 173, 71, 142, 0,
	 ...
	 *                                    select                                                                   *
	 *----------------------------------------------------------------------------------------*/
        static vector<size_t> Select(Generator& gen, size_t n, size_t total) {
		vector<size_t> out;
		out.reserve(n);
	 ...
	 *                                    combine                                                                  *
	 *----------------------------------------------------------------------------------------*/
	static void Combine(const Container& data, uint32_t seed, uint8_t field, uint8_t sparsity, Frame& out) {
		/**
		 * coefficient generator
		 */
		Generator c(seed);
		 ...
		 */
		Frame aux;
		aux.reserve(out.capacity());
                /**
                 */
                for(register auto i : Select(c, data.size(), sparsity)){
                        /**
			 * Y += Xn*Cn
			 */
			Merge(out, Mul((aux = data[i]), field & uint8_t(c())));
                }
	}
