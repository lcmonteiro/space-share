 ...
 *  encode functions
 */
static inline uint32_t Combine(const Container& data, uint32_t seed, uint8_t field, uint8_t sparsity, Frame& out);
/**
 * decode functions
 */
...
#define ENCODE(MIN, FIELD, SPARSITY, FRAMESIZE) do{                                                 \
        ...
        for (unsigned int i = highDensity; i < size; i++) {
                ENCODE(1, stamp[uint8_t(seed)].first, stamp[uint8_t(seed)].second, FRAME_SIZE);
        }
        /**
         * high density
         */
 ...
 *                                    combine                                                                          *
 *---------------------------------------------------------------------------------------------------------------------*/
static uint32_t Combine(const Container& data, uint32_t seed, uint8_t field, uint8_t sparsity, Frame& out) {
        /**
         * coefficient generator
         */
        Generator c(seed);
